...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const hasExtendedFilters = (formCodes && formCodes.length > 0) || (routeCodes && routeCodes.length > 0) || (reimbursementCategories && reimbursementCategories.length > 0) || priceMin !== undefined || priceMax !== undefined || chapterIV !== undefined || chapterIVParagraph !== undefined || deliveryEnvironment !== undefined || medicineType !== undefined;
    const hasFilters = hasBasicFilters || hasExtendedFilters;
    const filters: SearchFilters | undefined = hasFilters
      ? { vtmCode, vmpCode, ampCode, atcCode, companyCode, vmpGroupCode, substanceCode, reimbursable, blackTriangle, formCodes, routeCodes, reimbursementCategories, priceMin, priceMax, chapterIV, chapterIVParagraph, deliveryEnvironment, medicineType }
      : undefined;
