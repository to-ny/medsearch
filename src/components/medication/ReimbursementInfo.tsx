...
interface ReimbursementInfoProps {
  ...
  /** CNK code for fetching Chapter IV details (required for progressive disclosure) */
  cnk?: string;
}

export function ReimbursementInfo({ reimbursements, medicationPrice, cnk }: ReimbursementInfoProps) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        {/* Chapter IV Info Box and Details */}
        {isChapterIV && (
          <div className="space-y-4">
            <ChapterIVInfoBox />
            {cnk && <ChapterIVDetails cnk={cnk} />}
          </div>
        )}
        {/* Category */}
        {reimbursement.criterion && (
          <div>
            ...
            <p className="text-lg text-gray-900 dark:text-white">
              {['A', 'B', 'C', 'Cs', 'Cx', 'D'].includes(reimbursement.criterion.category)
                ? t(`reimbursement.categories.${reimbursement.criterion.category as 'A' | 'B' | 'C' | 'Cs' | 'Cx' | 'D'}`)
                : `Category ${reimbursement.criterion.category}`}
            </p>
          </div>
        )}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-lg bg-blue-50 p-4 dark:bg-blue-900/20">
            ...
            <p className="text-3xl font-bold text-blue-700 dark:text-blue-300">
              {patientCost !== undefined ? formatPrice(patientCost) : t('reimbursement.varies')}
            </p>
          ...
          <div className="rounded-lg bg-green-50 p-4 dark:bg-green-900/20">
            ...
            <p className="text-3xl font-bold text-green-700 dark:text-green-300">
              {insurancePays !== undefined ? formatPrice(insurancePays) : t('reimbursement.varies')}
            </p>
                ...
                .map((copay, index) => (
                  <div key={index} className="flex justify-between">
                    <span className="text-gray-500 dark:text-gray-400">
                      {['PREFERENTIAL', 'REGULAR', 'UNKNOWN'].includes(copay.regimen)
                        ? t(`reimbursement.regimen.${copay.regimen as 'PREFERENTIAL' | 'REGULAR' | 'UNKNOWN'}`)
                        : copay.regimen}
                    </span>
